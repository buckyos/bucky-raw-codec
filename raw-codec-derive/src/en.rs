...
fn encode_enum_inner_variant(
    ...
    match variant.style {
        Style::Unit => (
            quote_block!({
                let __buf__ = bucky_raw_codec::USize(#variant_index).raw_encode(__buf__, __purpose__)?;
                Ok(__buf__)
            }),
            quote_block!({
                Ok(bucky_raw_codec::USize(#variant_index).raw_measure(__purpose__)?)
            }),
        ),
        Style::Newtype => {
            ...
            (
                if optimize_option && _field.is_option() {
                    ...
                    let item = quote_block! {{
                        ...
                        #flag
                        let __buf__ = {
                            let __buf__ = bucky_raw_codec::USize(#field_expr.len()).raw_encode(__buf__, __purpose__)?;
                            if #field_expr.len() == 0 {
                                __buf__
                            } else {
                                unsafe {
                ...
                if optimize_option && _field.is_option() {
                    quote_block! {{
                        Ok(bucky_raw_codec::USize(#variant_index).raw_measure(__purpose__)? + #flag_measure + if #field_expr.is_some() {#field_expr.as_ref().unwrap().raw_measure(__purpose__)?} else {0})
                    }}
                } else if _field.is_vec_u8() {
                    quote_block!({
                        Ok(bucky_raw_codec::USize(#variant_index).raw_measure(__purpose__)? + #flag_measure + bucky_raw_codec::USize(#field_expr.len()).raw_measure(__purpose__)? + #field_expr.len())
                    })
                } else {
                    quote_block!({
                    Ok(bucky_raw_codec::USize(#variant_index).raw_measure(__purpose__)? + #flag_measure + #field_expr.raw_measure(__purpose__)?)
                    })
                },
            )
        }
        ...
        TupleVariant::ExternallyTagged {
            ...
            if encode_stmts.len() == 0 {
                ...
                (
                    ...
                    quote_block!({
                        Ok(bucky_raw_codec::USize(#variant_index).raw_measure(__purpose__)? + #flag_measure + #(#measure_encode_stmts)+*)
                    }),
                )
            }
        }
        ...
        StructVariant::ExternallyTagged {
            ...
            if encode_fields.len() == 0 {
                ...
                (
                    ...
                    quote_block!({
                        Ok(bucky_raw_codec::USize(#variant_index).raw_measure(__purpose__)? + #flag_measure + #(#measure_fields)+*)
                    }),
                )
            }
        }
     ...
     fields
        ...
        .map(|(i, field)| {
            ...
            let ser = if optimize_option && field.is_option() {quote! {
                ...
                quote!{bucky_raw_codec::USize(#field_expr.len()).raw_measure(__purpose__)? + #field_expr.len()}
            } else {
             ...
             .map(|field| {
                 ...
                 if optimize_option && field.is_option() {
                    quote! {
                     ...
                     let item = quote! {
                        let __buf__ = {
                            let __buf__ = bucky_raw_codec::USize(#member.len()).raw_encode(__buf__, __purpose__)?;
                            if #member.len() == 0 {
                                __buf__
                            } else {
                                unsafe {
         ...
         flag_measure, fields
             ...
             .map(|field| {
                 ...
                 if optimize_option && field.is_option() {
                     ...
                     quote!{bucky_raw_codec::USize(#member.len()).raw_measure(__purpose__)? + #member.len()}
                 } else {
                     quote! {
            ...
            .filter(|field| !field.attrs.skip_serializing())
             .map(|field| {
                 ...
                 if optimize_option && field.is_option() {
                     ...
                     let item = quote! {
                        let __buf__ = {
                            let __buf__ = bucky_raw_codec::USize(self.#member.len()).raw_encode(__buf__, __purpose__)?;
         ...
         flag_measure, fields
             ...
             .map(|field| {
                 ...
                 if optimize_option && field.is_option() {
                     ...
                     quote!{bucky_raw_codec::USize(self.#member.len()).raw_measure(__purpose__)? + self.#member.len()}
                 } else {
                     quote! {
